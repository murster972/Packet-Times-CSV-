...
#	   allow the user to choose paramaters for vlaid packets, i.e source, dest, ports, length, time
...
def find_gaps(time_dir, max_diff=0):
	...
	while n[1] < len(keys):
		...
		#if diff >= max_diff: packs.append(cur_keys)
...
def draw_graph(data):
	#graphs time for ech packet to be sent
	#packet numbers
	...
	max_y = 0
	for i in data:
		max_y = float(i[1]) if float(i[1]) > max_y else max_y
	min_y = max_y
	for i in data:
		min_y = float(i[1]) if float(i[1]) < min_y else min_y
...
def main():
	...
	time_diffs = [[x[0], x[2]] for x in problem_packets]
